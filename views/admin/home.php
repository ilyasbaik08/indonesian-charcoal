...
<body class="bg-gray-100 text-white font-poppins">
  <nav class="fixed top-0 left-0 right-0 z-50 bg-white py-4 px-6 flex justify-between items-center border-b border-gray-700">
    <a href="../../controllers/LogoutController.php" class="text-black text-2xl font-bold italic">Indonesian<span class="text-cyan-500"> Charcoal</span></a>
    <div class="flex space-x-4 items-center">
      <span class="text-black hidden md:block"><?= htmlspecialchars($user['name']); ?></span>
      <a href="#" id="button-modal" class="text-black hover:text-cyan-500"><i data-feather="user"></i></a>
      <a href="#" id="menu-button" class="text-black md:hidden hover:text-cyan-500"><i data-feather="menu"></i></a>
      <a href="#" id="close-button" class="text-black hidden md:hidden hover:text-cyan-500"><i data-feather="x"></i></a>
    </div>
  </nav>
  ...
  <div class="absolute right-10 top-20 cursor-pointer">
    <button id="modal-button" class="py-2 px-4 bg-cyan-500 hover:bg-cyan-400 text-xs">Tambah Produk</button>
  </div>
  ...
  <div id="menu-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center px-12 z-50 opacity-0 pointer-events-none transition-opacity duration-500 ease-in-out">
    <form action="../../controllers/AddMenuController.php" method="POST" enctype="multipart/form-data" class="relative bg-white/60 backdrop-blur-lg p-6 rounded-lg shadow-lg w-full max-w-md">
      ...
      <input type="text" id="menu-name" name="menu_name" class="w-full p-2 mt-1 focus:outline-none text-gray-900">
      ...
      <?php foreach ($menus as $menu): ?>
        <div class="bg-white p-6 rounded-lg shadow-lg flex flex-col items-center">
          <div class="flex justify-center gap-4 mb-4">
            <a href="../../controllers/DeleteMenuController.php?id=<?= $menu['id']; ?>" class="text-cyan-500 bg-gray-100 p-3 rounded-full hover:bg-cyan-500 hover:text-gray-100 transition duration-300 ease-in-out">
              <i data-feather="trash-2"></i>
            </a>
            <a href="#" class="bg-cyan-500 p-3 hover:bg-cyan-400 transition duration-300 ease-in-out flex items-center justify-center edit-button"
